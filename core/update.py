...
class MiniConvGRU(nn.Module):
    def __init__(self, hidden_dim, input_dim, kernel_size=3, num_groups=1, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.conv1 = nn.Conv2d(input_dim, hidden_dim, kernel_size, padding=kernel_size//2, groups=num_groups)
        self.conv2 = nn.Conv2d(input_dim, hidden_dim, kernel_size, padding=kernel_size//2, groups=num_groups)

    def forward(self, h, cz, cq, *x_list):
        x = torch.cat(x_list, dim=1)
        z = torch.sigmoid(self.conv1(x) + cz)
        q = F.tanh(self.conv2(x) + cq)
        h = (1-z) * h + z * q
        return h
...
class UpdateBlock(nn.Module):
    def __init__(self, args, hidden_dims=[]):
        ...
        self.gru04 = MiniConvGRU(hidden_dims[2], encoder_output_dim + hidden_dims[1] * (args.n_gru_layers > 1))
        self.gru08 = MiniConvGRU(hidden_dims[1], hidden_dims[0] * (args.n_gru_layers == 3) + hidden_dims[2])
        self.gru16 = MiniConvGRU(hidden_dims[0], hidden_dims[1])
