...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root=r'G:/dataset/StereoDatasets/sceneflow/', dstype='frames_finalpass', things_test=False):
    ...
    def _add_monkaa(self, split="TRAIN"):
        ...
        original_length = len(self.disparity_list)
        root = self.root
        left_images = sorted( glob(osp.join(root, 'Mondaa', self.dstype, '*/left/*.png')) )
...
class Middlebury(StereoDataset):
    def __init__(self, aug_params=None, root='D:\StereoDatasets-238G\muddlebury', split='F'):
